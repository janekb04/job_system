...
template <typename T, bool AcceptsExceptions = true> class value_or_exception;
template <> class value_or_exception<void, false> {
public:
  constexpr void get() noexcept {}
};
template <> class value_or_exception<void, true> {
private:
  std::exception_ptr exception;

public:
  value_or_exception() noexcept : exception{} {}
...
template <typename T> class store_as_union {
protected:
  union {
    T value;
    std::exception_ptr exception;
...
public:
  constexpr store_as_union() noexcept : has_exception{false} {}
};
template <typename T> class store_as_pair {
protected:
  alignas(alignof(T)) char value[sizeof(T)];
  std::exception_ptr exception;

public:
  constexpr store_as_pair() noexcept : exception{} {}
};
template <typename T> class value_or_exception<T, false> {
private:
...
public:
  constexpr value_or_exception() noexcept : store_as_union<T>{} {}
  constexpr void
  set_value(T &&val) noexcept(std::is_nothrow_move_constructible_v<T>) {
...
public:
  constexpr value_or_exception() noexcept : store_as_pair<T>{} {}
  constexpr void
  set_value(T &&val) noexcept(std::is_nothrow_move_constructible_v<T>) {
    new (static_cast<T *>(static_cast<void *>(&store_as_pair<T>::value)))
        T{std::move(val)};
  }
  constexpr void set_exception(std::exception_ptr &&ptr) noexcept(
      std::is_nothrow_move_assignable_v<std::exception_ptr>) {
