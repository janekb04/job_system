...
public:
  ALWAYS_INLINE void add_ready_to_launch(task &t) { ready->push_back(t); }
  ALWAYS_INLINE void add_ready_to_resume(task &t) { add_ready_to_launch(t); }
  ALWAYS_INLINE task &get_next_task() {
    task &t = ready->pop_front();
    return t;
  }
  ALWAYS_INLINE bool done() { return ready->empty(); }
